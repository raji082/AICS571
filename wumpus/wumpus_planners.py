...
class PlanRouteProblem(search.Problem):
    ...
    def h(self, node):
        ...
        """
        distance=[]
        #Iterate through all the goals and find the min distance and take that value as heuristic
    ...
    def actions(self, state):
        ...
        """
        #From any state we can move either left or right or move forward
        possible_actions = ['TurnLeft','TurnRight', 'Forward','Grab', 'Climb', 'Shoot']
        #Take the direction of the current state
        direction = state[2]
        if direction == 0 : #north
            #if the next state (x+1,y) is in allowed states then we can move either left or right or move forward
            if (state[0], state[1]+1) in self.allowed:
                actione = possible_actions[0:3]
            else:
            # if the next state (x+1,y) is not in allowed states then we can move either left or right
                actione = possible_actions[0:2]
        elif direction == 1: #west
            # if the next state (x-1,y) is in allowed states then we can move either left or right or move forward
            if (state[0]-1, state[1]) in self.allowed:
                actione = possible_actions[0:3]
            else:
            # if the next state (x-1,y) is in allowed states then we can move either left or right
                actione = possible_actions[0:2]
        elif direction == 2 : #south
            # if the next state (x,y-1) is in allowed states then we can move either left or right or move forward
            if (state[0], state[1]-1) in self.allowed:
                actione = possible_actions[0:3]
            # if the next state (x-1,y) is in allowed states then we can move either left or right
            else:
    ...
    def result(self, state, action):
        ...
        3 = East
        """
        #get the current direction of the state
        current_direction = state[2]
        if current_direction == 0: #north
            if action == 'TurnLeft':
                ...
                new_state = (state[0], state[1]+1, 0)
        elif current_direction == 1: #west
            if action == 'TurnLeft':
    ...
    def goal_test(self, state):
        ...
        """
        #iIf the current state is in goals then return True else return False
        for goal in self.goals:
...
class PlanShotProblem(search.Problem):
    ...
    def h(self, node):
        ...
        """
        distance = []
        #In PlanShotProblem, we iterate only through the nodes which are facing the wumpus. This means the nodes which have either x co-ordinate or y co-ordinates of
        #the wumpus are iterated. We then find the one with min distance to a goal.
        for goal in self.goals:
            for allowedstate in self.allowed:
                if (goal[0] == allowedstate[0] ) or (goal[1] == allowedstate[1]):
                     distance.append(manhattan_distance_with_heading(node.state, allowedstate))
        heuristic = min(distance)
        return heuristic
    ...
    def actions(self, state):
        ...
        """
        # From any state we can move either left or right or move forward
        possible_actions = ['TurnLeft', 'TurnRight', 'Forward']
        #Take the direction of the current state
        direction = state[2]
        if direction == 0:  # north
            # if the next state (x+1,y) is in allowed states then we can move either left or right or move forward
    ...
    def goal_test(self, state):
        ...
        Return True if state is a goal state
        """
        #current coordinates of the state
        current_direction = state[2]
        # current direction of the state
        current_state = state[0:2]
        #Iterate through the goals and return true if coordinates of the state are facing any of the goals else return False
        for goal in self.goals:
            if ((goal[0] == current_state[0] and goal[1] > current_state[1] and current_direction == 0) or
                    (goal[0] < current_state[0]  and goal[1] == current_state[1] and current_direction == 1) or
                    (goal[0] == current_state[0] and goal[1] < current_state[1]  and current_direction == 2) or
                    (goal[0] > current_state[0]  and goal[1] == current_state[1] and current_direction == 3)):
                return True
        return False
